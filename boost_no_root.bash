...
bash bootstrap.sh
./b2 -j16 toolset=gcc --build-type=complete link=shared runtime-link=shared threading=multi,single variant=debug,release address-model=64 --layout=tagged --without-python --cxxflags="-std=c++17" install stage 2>&1 | tee build.log
ldconfig